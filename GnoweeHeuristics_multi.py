...
class GnoweeHeuristics_multi(ProblemParameters_multi):
    ...
    ##
    def __init__(self, population=100, initSampling='lhc', fracMutation=0.2,
                 fracElite=0.2, fracLevy=1.0, alpha=0.5, gamma=1, n=1,
                 scalingFactor=10.0, penalty=0.0, maxGens=20000,
                 maxFevals=10000, convTol=1e-6, stallLimit=5000,
    ...
    def population_update_multi(self, parents, children, timeline=None,
        ...
        @return <em> list of history objects: </em> If an initial timeline was
            ...
            evaluations.
        """
        if self.BadVectorDetermination == True:
            from UserBadVectorDetermination import getVectorDetermination
        ...
        if numFunctions == 1:
            ...
            if self.BatchSubmit== True:
                ...
                if self.BadVectorDetermination == True: #if using the bad vector determination step
                    acceptabilityList = getVectorDetermination(weeded_Children)
                    badChildren = []
                    goodChildren = []
                    for i in range(0,len(weeded_Children)):
                        if acceptabilityList[i] == 0: #if the child was determined to be unaccaptable
                            badChildren.append(weeded_Children[i])
                        else: #if the child is good
                            goodChildren.append(weeded_Children[i])
                else: #if not using the bad vector determination step
                    acceptabilityList = []
                    for i in range(0,len(weeded_Children)):
                        acceptabilityList.append(1)
                    goodChildren = weeded_Children
                    badChildren = []
                #send these feature fectors to the batch submission system (aasumes user has specified a translate_feature_vector system
                outputFileNames = Batch_Submit(goodChildren, self.templateFile, prefix = self.prefix)
                num_children_to_eval = len(goodChildren)
                for i in range(0,num_children_to_eval):
                    newfit = self.objective.func(goodChildren[i], outputFileNames[i])
                    self.LibInterface.add_to_lib(goodChildren[i], newfit)
                #handle the bad children and their "results"
                if self.BadVectorDetermination == True: #if using the bad vector determination step
                    if self.penalty == 0.0: #if the penalty was left as the stupid default
                        tempPenalty = 1.0
                    else: #if the penalty was set manually, use that
                        tempPenalty = self.penalty
                    for i in range(0, len(badChildren)):# go through and assign fittnesses to the children whose feature vectors could not be evaluated in the standard way
                        newfit = tempPenalty #works because this is the single objective function branch
                        self.LibInterface.add_to_lib(badChildren[i], newfit)
                #save the results fo the evaluation step
            ...
            for i in range(0, len(children), 1):
                ...
                feval += 1 #consider changing to be based on weeded children
            ...
            feval = num_children #consider changing to be based on weeded children

            #perform some clean up that may be required if the BatchSubmit process was used in runNSGA_generation
