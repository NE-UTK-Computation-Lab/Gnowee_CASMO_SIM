...
# added some comments
# in the multi-objective case, penalties need to be returned as a 
# vector whose entries are ordered in the same fashion as the fitness
# function values they apply to
# to assist in this task, and to make sure the code responds the same way
# in the single objective case, an additional variable is added to __init__ that
# specifies the number of objective functions and modifies the penalty veriable
#-----------------------------------------------------------------------------#
class Constraint_Multi(object):
    """!
    @ingroup Constraints
    ...
    def __init__(self, method=None, constraint=None, penalty=1E15, NumberObjectiveFunctions = 1):
        """!
        ...
        # is violated, a vector(ndarray specfically) if there 
        # is more than one objective function
        self.penalty = penalty

        ## @var NumberObjectiveFunctions
        # \e int: The number of objective functions
        self.NumberObjectiveFunctions = NumberObjectiveFunctions

        if NumberObjectiveFunctions > 1:
            if not(isinstance(penalty, np.ndarray)):
                self.penalty = penalty * np.ones(NumberObjectiveFunctions)
...
# directory.
#
# Comments added by Alex:
# Constaints are implemented as a set of expressions, each of which individually
# is considered a "constraint"
# these mathematical expressions can be inequalities and are functions of the 
# design variables. For example:
# g_1(x_vec) <= C
# g_2(x_vec) = C
# it seems that in the docummentation and functions below:
# first, g_1, g_2, ect are evaluated at a possible particular solution (x_vec, represented by u)
# then the g functions are assembled into a list 
# second, a violation size is determined by calling less_or_equal, less_than, or greater_than
# and comparing against C (in the example above), which is typically set to zero
# C is a float and is stored as self.constraint (not to be confuessed with the constraint functions (g_1,g_2, ect); 
# the evaluated constraint functions (g_1(x_vec),g_2(x_vec), ect); or the inequality expressions). It seems that only one 
# value for C can be accepted (in a given Constraint Object). 
#
# worryingly, the functions below use the line:
# self.get_penalty(self.less_or_equal(constraint))
# where the get_penalty function performs:
# return self.penalty*ceil(violation)**2
# and the less_or_equal function performs:
# return self.get_penalty(candidate-self.constraint)
# thus the line:
# self.get_penalty(self.less_or_equal(constraint))
# becomes effectivly:
# self.get_penalty(self.get_penalty(g(x_vec)-self.constraint))
# which simplifies to:
# self.get_penalty(self.penalty*ceil(g(x_vec)-self.constraint)**2)
# then further simplifies to:
# self.penalty*ceil(self.penalty*ceil(g(x_vec)-self.constraint)**2)**2
# essentially applying the penalty scaling twice
#
# each of these functions (less_or_equal, less_than, or greater_than) takes
# in an evaluated constraint function (g_1(x_vec),g_2(x_vec), or ect)
# checks if it is less than, equal to, or greater than the specficed value C 
# and if it is, returns the penalty
#-----------------------------------------------------------------------------#
    def spring(self, u):
        """!
    ...
    def Test_multi_objective_constraints_circle(self, u):
        vec = u
        x = vec[0]
        y = vec[1]
        f1 = x+y
        f2 = (x**2)+((y-2)**2)
        g = (f1**2)+(f2**2)-25
        total_penalty = self.get_penalty(self.greater_than(g)) * np.ones(self.NumberObjectiveFunctions)
