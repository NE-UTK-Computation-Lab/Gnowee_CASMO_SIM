...
class ProblemParameters_multi(object):
    ...
    def __init__(self, objective=None, constraints=[], lowerBounds=[],
                 ...
                 templateFile = "batch_test.txt", prefix = "MCNP_File_Num", BadVectorDetermination = False):
        """!
        ...
        #1-11-2022
        #added BadVectorDetermination Keyword
        #also, I don't remember when, but I also added the BatchSubmit Keyword
        #the BadVectorDetermination keyword, if set to True, will trigger
        # the import of UserBadVectorDetmination.py
        #which is a user supplied, file that contains the function "getVectorDeterminations(Feature_Vector_List):"
        # this function must have the following characteristics:
        #INPUTS:
        #Feature_Vector_List: a list of feature vectors
        #OUTPUTS:
        #BadVectorArray: a list of the same length as Feature_Vector_List, that contains ones (1)s or zeros (0)
        # coresponding to either acceptable of unaccaptable feature vectors (respectively), this list should have the same order as Feature_Vector_List
        # feature vectors deemed unacceptable will not be evaluated and will be treated as having a huge penalty
        self.BadVectorDetermination = BadVectorDetermination
        ...
        if self.numObjectiveFunctions == 1: #because right now the result save system is only integrated with the single objective system
            ...
            #start integrating multiobjectivity with NSGA and Batch Submission
            self.LibInterface = resultsLibray_Interface(FileName, ColumnNames, keyColumnIndex, ValueColumnIndex, Filelocation,  numObjectiveFunctions = self.numObjectiveFunctions)
        ## @var constraints
