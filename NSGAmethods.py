...
def runNSGA_generation(P_t, Q_t, problemParametersObject):
    ...
    if problemParametersObject.BatchSubmit == True:
        ...
        children = Q_t
        weeded_Children = problemParametersObject.LibInterface.weed_list(children)
        num_Weeded_Children = len(weeded_Children)
        #send these feature fectors to the batch submission system (aasumes user has specified a translate_feature_vector system)
        outputFileNames = Batch_Submit(weeded_Children, problemParametersObject.templateFile, prefix = problemParametersObject.prefix)
        num_children_to_eval = len(weeded_Children)
        for k in range(0,num_children_to_eval):
            newfit = []
            for i in range(0, num_objectives):
                newfit.append(problemParametersObject.objective[i].func(weeded_Children[k], outputFileNames[k])) #call the evaluation function in the multiobjective way (with the file name as an arguement)
            problemParametersObject.LibInterface.add_to_lib(weeded_Children[k], newfit)
        problemParametersObject.LibInterface.SaveLib()
        for file in os.listdir():
            if problemParametersObject.prefix in file:
                if ".inp" in file:
                    os.remove(file)
        #this step converts the results from the batch submission system to a form that the rest of the NSGA program will work with
        for k in range(0, num_children):
            #print 'k = ', k
            for i in range(0, num_objectives):
                #print 'i = ',
                child_list[k].fitness[i]= problemParametersObject.LibInterface.getFitness(children[k], i) #replaced the straightforward calculation with a library look up

            child_list[k].Evaluated = 1

    else:
        #traditional method (no batch submit)
        for k in range(0, num_children):
            #print 'k = ', k
            for i in range(0, num_objectives):
                #print 'i = ', i
    ...
    front_index = 0
    print("N:" +str(N))
    Max_num_fronts = len(sorted_population.fronts)
    #sorted_population.fronts.append([])
    print("Number of Fronts in sorted_population:" +str(len(sorted_population.fronts)))
    for i in range(0, Max_num_fronts):
        number_members = len(sorted_population.fronts[i])
        print("Front number "+str(i)+" has " +str(number_members)+" members")
    ...
    while continue_adding == True:
        ...
        print("Front index: "+str(front_index))
        print("Length of new_population:" +str(len(new_population)))
        front_index = front_index + 1 #RETHINK
        if len(new_population) == N:
            continue_adding = False
        elif len(new_population) + len(sorted_population.fronts[front_index]) > N:
            continue_adding = False
        #front_index = front_index + 1 #RETHINK
    oldWay =False
    if oldWay == True:
